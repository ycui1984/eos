...
static struct dentry *autofs4_lookup_active(struct dentry *dentry)
{
	...
	list_for_each(p, head) {
		...
		mode = acquire_mixed_lock(&active->d_lock, &node);
		...
		if (d_unhashed(active)) {
			dget_dlock(active);
			release_mixed_lock(&active->d_lock, &node, mode);
			spin_unlock(&sbi->lookup_lock);
			return active;
		}
next:
...
static struct dentry *autofs4_lookup_expiring(struct dentry *dentry)
{
	...
	list_for_each(p, head) {
		...
		mode = acquire_mixed_lock(&expiring->d_lock, &node);
		...
		if (d_unhashed(expiring)) {
			dget_dlock(expiring);
			release_mixed_lock(&expiring->d_lock, &node, mode);
			spin_unlock(&sbi->lookup_lock);
			return expiring;
		}
next:
