...
void percpu_counter_set(struct percpu_counter *fbc, s64 amount)
{
	...
	mode = acquire_mixed_lock(&fbc->lock, &node);
	...
	release_mixed_lock(&fbc->lock, &node, mode);
}
					...
					unsigned long action, void *hcpu)
{
	...
	list_for_each_entry(fbc, &percpu_counters, list) {
		...
		/* flags = __raw_spin_lock_irqsave(&fbc->lock); */
		...
		*pcount = 0;
		...
		/* __raw_spin_unlock_irqrestore(&fbc->lock, flags); */
	}
	spin_unlock(&percpu_counters_lock);
#endif
	return NOTIFY_OK;
