...
static void sel_remove_entries(struct dentry *de)
{
	...
	volatile struct qnode node1, node2;
	enum release_mode mode1, mode2;

	mode1 = acquire_mixed_lock(&de->d_lock, &node1);
	node = de->d_subdirs.next;
	while (node != &de->d_subdirs) {
		struct dentry *d = list_entry(node, struct dentry, d_u.d_child);

		mode2 = acquire_mixed_lock(&d->d_lock, &node2);
		list_del_init(node);

		if (d->d_inode) {
			dget_dlock(d);
			release_mixed_lock(&de->d_lock, &node1, mode1);
			release_mixed_lock(&d->d_lock, &node2, mode2);
			d_delete(d);
