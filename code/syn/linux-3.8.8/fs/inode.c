...
__cacheline_aligned_in_smp struct lock inode_sb_list_lock = {
        .tas_lock = UNLOCKED,
...
void address_space_init_once(struct address_space *mapping)
{
	...
	init_lock(&mapping->i_mmap_mutex);
#else
...
void inode_sb_list_add(struct inode *inode)
{
	...
	mode = acquire_mixed_lock(&inode_sb_list_lock, &node);
	list_add(&inode->i_sb_list, &inode->i_sb->s_inodes);
	release_mixed_lock(&inode_sb_list_lock, &node, mode);
}
