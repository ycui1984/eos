...
{
	int size, specsize, offset, i, j, k, l;
	char *buf, *specbuf, *p;
	...
	if (6 != sscanf(p, "%lu%d%d%d%d%n", &record.best, &record.completed, &record.dim_loc, &record.loc.enumloc, &record.to_value, &offset)) {
	...
	for (i = 0; i < perfcache.guard; i++) {
		if (2 != sscanf(p, "%d%lu%n", &perfcache.perfc[i].fp.rw_ratio, &perfcache.perfc[i].fp.avg_size, &offset)) {
		...
		if (6 != sscanf(p, "%lu%d%d%d%d%n", &perfcache.perfc[i].rcd.best, &perfcache.perfc[i].rcd.completed, &perfcache.perfc[i].rcd.dim_loc, &perfcache.perfc[i].rcd.loc.enumloc, &perfcache.perfc[i].rcd.to_value, &offset)) {
			printk("not enough info\n");
			return;
		}
		p += offset;
...
static __exit void auto_exit(void)
{
	...
	for (i = 0; i< perfcache.guard; i++) {
		...
		for (j = 0; j < vector.dim_guard; j++) {
			p += sprintf(p, "%d\t", perfcache.perfc[i].conf[j]);
		} 
		p += sprintf(p, "\n");

		for (j = 0; j < vector.dim_guard; j++) {
			p += sprintf(p, "%d\t", perfcache.perfc[i].bestconf[j]);
		}
		p += sprintf(p, "\n");

		p += sprintf(p, "%lu\t", perfcache.perfc[i].best);
		p += sprintf(p, "%d\t", perfcache.perfc[i].completed);
		p += sprintf(p, "%d\t", perfcache.perfc[i].dim_loc);
		p += sprintf(p, "%d\n", perfcache.perfc[i].loc.enumloc);
		p += sprintf(p, "%d\n", perfcache.perfc[i].to_value);
	}
#endif
